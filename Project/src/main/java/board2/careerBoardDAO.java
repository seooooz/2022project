...
public class careerBoardDAO extends DBConnPool {
	...
	public ArrayList<careerBoardDTO> selectBoards() {
		String sql = "select * from " + " ( " + "	SELECT * FROM careerboard " + "	ORDER  BY cnum DESC ) "
				+ " where rownum <= 5";
	...
	public ArrayList<careerBoardDTO> getList(int num) {
		String sql = "SELECT * FROM ( " + "	SELECT * FROM careerboard " + "	WHERE  cnum < ? " + "	ORDER BY cnum DESC "
				+ " ) " + " WHERE rownum <=10";
	...
	public void updateVisitCount(String num) {
		String sql = "update careerboard set " + " cvisitcount = cvisitcount+1 " + " where cnum=?";
		System.out.println(sql);
		try {
			psmt = con.prepareStatement(sql);
	...
	// mypage - carreaboard출력
	public ArrayList<careerBoardDTO> selectcareerView(String id) {

		String sql = "select * from careerboard where cid = ? ORDER BY cnum DESC ";
		ArrayList<careerBoardDTO> list = new ArrayList<careerBoardDTO>();
